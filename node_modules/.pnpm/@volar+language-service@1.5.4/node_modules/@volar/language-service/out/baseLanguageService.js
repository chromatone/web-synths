...
function createLanguageService(modules, env, config, host, documentRegistry) {
    const languageContext = (0, language_core_1.createLanguageContext)(modules, host, Object.values(config.languages ?? {}).filter(common_1.notEmpty));
...
function createLanguageServicePluginContext(modules, env, config, host, languageContext, documentRegistry) {
    const ts = modules.typescript;
    let tsLs;
    if (ts) {
        const created = tsFaster.createLanguageService(ts, languageContext.typescript.languageServiceHost, proxiedHost => ts.createLanguageService(proxiedHost, documentRegistry), languageContext.typescript.languageServiceHost.getCurrentDirectory());
        ...
        if (created.setPreferences && env.getConfiguration) {
            ...
            async function updatePreferences() {
                const preferences = await env.getConfiguration?.('typescript.preferences');
                if (preferences) {
    ...
    function getTextDocument(uri) {
        ...
        if (scriptSnapshot) {
            ...
            if (!document) {
                ...
                document = vscode_languageserver_textdocument_1.TextDocument.create(uri, host.getScriptLanguageId?.(fileName) ?? (0, common_1.resolveCommonLanguageId)(uri), newVersion, scriptSnapshot.getText(0, scriptSnapshot.getLength()));
