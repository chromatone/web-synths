    ...
    if (virtualFile) {
        await (0, definePlugin_1.visitEmbedded)(context.documents, virtualFile, async (file, map) => {
            ...
            for (const ruleId in context.config.rules) {
                ...
                const embeddedResult = await safeCall(() => worker(ruleId, rule, ruleCtx), 'rule ' + ruleId + ' crashed on ' + map.virtualFileDocument.uri);
                if (!embeddedResult)
                    continue;
                const result = transform(embeddedResult, map);
                if (!result)
        ...
        for (const ruleId in context.config.rules) {
            ...
            ruleCtx.ruleId = ruleId;
            const embeddedResult = await safeCall(() => worker(ruleId, rule, ruleCtx), 'rule ' + ruleId + ' crashed on ' + document.uri);
            if (!embeddedResult)
                continue;
            const result = transform(embeddedResult, undefined);
            if (!result)
