...
function register(context) {
    ...
    return async (uri, mode, token = vscode.CancellationToken.None, response) => {
        ...
        async function lintWorker(ruleType, cacheMap, cache) {
            const result = await (0, featureWorkers_1.ruleWorker)(context, ruleType, uri, file => ruleType === types_1.RuleType.Format ? !!file.capabilities.documentFormatting : !!file.capabilities.diagnostic, async (ruleName, rule, ruleCtx) => {
                if (token) {
                ...
                const tsProjectVersion = (ruleType === types_1.RuleType.Semantic) ? context.core.typescript.languageServiceHost.getProjectVersion?.() : undefined;
