    ...
    return async (uri, range, legend, token = vscode.CancellationToken.None, reportProgress) => {
        ...
        const tokens = await (0, featureWorkers_1.languageFeatureWorker)(context, uri, offsetRange, function* (offsetRange, map) {
            ...
            if (token?.isCancellationRequested)
                return;
            return service.provideDocumentSemanticTokens?.(document, vscode.Range.create(document.positionAt(offsetRange[0]), document.positionAt(offsetRange[1])), legend, token);
